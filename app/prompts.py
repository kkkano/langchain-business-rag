    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "如果回答中包含结论，请在相关句子后附带 [source_id] 形式的引用标记。"
            ...
            (
                ...
                "用户问题：{question}\n\n"
                "可用上下文：\n{context}\n\n"
                "请输出一个 JSON 对象，包含 answer、grounded、citations 三个字段。"
                "citations 必须是数组，每项包含 source_id、source_name、segment_label、supporting_text。"
                "citations 里的 source_id 必须原样引用上下文中的 source_id。"
    ...
    return ChatPromptTemplate.from_messages(
        [
            ...
            (
                ...
                "可用上下文：\n{context}\n\n"
                "请输出一个 JSON 对象，包含 answer、grounded、citations 三个字段。"
                "如果草稿成立，answer 字段应尽量保持原回答表达；如果草稿不成立，返回“我不知道”。",
            ),
        ]
    )
